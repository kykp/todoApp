...
export const LeftMenu = () => {
    ...
    const onHandleLeftMenuProjectsClick = (event) => {
        setAnchorPoint({ x: event.pageX, y: event.pageY });
    };
    useEffect(() => {
    ...
    return (<div className='left-menu'>
          ...
          {newProjects.map(el => (<div key={el.project} className='projects-list__cover'> 
              ...
              {activeButton === el.id &&
                ...
                el.id !== "archive" &&
                el.id !== "deleted" ?
                <img id={el.id} style={{ visibility: "visible" }} className='projects-list__pencil-img' src={PenImg} onClick={(e) => {
                        onHandlePopupProject();
                        onHandleLeftMenuProjectsClick(e);
                    }} alt="burger-menu"/>
                :
                    <img style={{ visibility: "hidden" }} className='projects-list__pencil-img' src={PenImg} alt="burger-menu"/>} 

            </div>))}
          </ul>
         
          <button className='button left-menu__button' onClick={(event) => onHandleAddNewProject(event)}>Добавить проект 
            </button>
