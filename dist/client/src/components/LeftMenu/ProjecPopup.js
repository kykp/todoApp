...
export const ProjecPopup = (props) => {
    ...
    return props.trigger ? (<div className="popup popup__left-menu">
      ...
      <button className='button left-menu__button popup-inner__menu' onClick={letsChangeNameProject}>Изменить
        </button>
      <button className='button left-menu__button popup-inner__menu' onClick={onHandleDeleteProject}>Удалить 
        </button>  
        </>
            : <>
        <p>Введите новое имя проекта</p>
        <input className='projecPopup__input' type="text" placeholder='Ведите название проекта' value={newProjectName} onChange={(e) => setNewProjectName(e.currentTarget.value)}/>
        <div className='left-menu__buttons'>
        <button className='button left-menu__button' onClick={props.onHandlePopupProject}>Отмена</button>
        <button className='button left-menu__button' onClick={onHandleChangeNameProject}>Ок</button>
            </div>
            </>} 
